...
`object`关键字创建的**伴生对象**，可以理解为替代Java的`static`关键字的方式，将静态方法用**单例对象**的实例方法做了替代，做到了更纯粹的面向对象。
...
- **变量和常量声明时，必须有初始值。**
- **变量可变，常量不可变。引用类型常量，不能改变常量指向的对象，可以改变对象的字段。**
...
- java中不是纯粹的面向对象（因为基本数据类型不是对象）。
- Scala吸取了这一点，所有数据都是对象，都是`Any`的子类。
- `Any`有两个子类：`AnyVal`值类型 `AnyRef`引用类型。
- 数值类型都是`AnyVal`子类，和Java数值包装类型都一样，只有整数在scala中是`Int`、字符是`Char`有点区别。StringOps`是java中`String`类增强，`AnyVal`子类。
...
- **Unit通常赋值或返回为AnyVal类的子类，Null是对AnyRef的子类来说。**
- **`Nothing` 确定没有正常的返回值，如抛异常时返回Nothing。通常不会指定Nothing为返回类，而是指定其他正常执行不抛异常的情况下的类。因为Nothing是所有类的子类，所以也被涵盖在其中（存在的意义）。**
```scala
object NullType {
...
```
Int 130 转 Byte 
计算机中存储数据都是以补码的形式来计算的，
对于正数来说原码、补码、反码三码合一，对于负数来说，
原码(最高位为符号位，除去符号位的正数部分乘个-1)  -> 除去符号位取反 -> 反码 -> +1 -> 补码 

那么对于 Int 130，在计算机中存储为补码
补码 0...(16个0) 1000 0010

Byte只有8位，截断最后8位得到补码1000 0010，然后要转成原码（按上面反着来）
反码（-1） 1000 0001
原码（除最高位取反） 1111 1110 =-126
...
```
- scala中特殊一点，`if-else`语句也有**返回值**，也就是说也可以作为表达式，定义为执行的最后一个语句的返回值。
...
- 范围步长`1 to 10 by 2`。倒序方法`10 to 1 by -2`. 
...
- 循环中同样可以**用`yield`返回**，生成一个新的集合类型来存储数据，外面可以接住用来操作，循环暂停，执行完后再继续循环。就像Ruby/Python。
    ```scala
...
- 使用`breakable`结构来实现`break continue`功能（其实就是try-catch方法）。
...
```
- 函数式编程语言中，函数是一等公民（可以像对象一样赋值、作为参数返回值），可以在任何代码块中定义函数，理解为子程序。
- 一般将定义在类或对象中（最外层）的函数称为方法（同java），而定义在方法中（内层）的称为函数（java不能在方法中再定义函数）。广义上都是函数。
...
```scala
    ...
    val f = foo(10)   // 结果：foo... 传的是函数的返回值
    println(f)      //()

    val f1= foo _   // 传整个函数对象
    println(f1)     // ScalaTest$$$Lambda$5/1289479439@66d33a
    println(f1(10))   // foo... ()

    val f2: Int => Unit = foo   // 传整个函数对象
    println(f2)             // ScalaTest$$$Lambda$6/2096171631@7e0babb1
    println(f2(10))           // foo... ()
```
...
高阶函数举例：
- 使用特定操作处理数组元素，得到新数组。也就是集合处理的map（映 射）操作。
```scala
...
- scala中的两种包管理方式：
    ...
    - 嵌套风格好处：
        - 一个源文件可以声明多个并列的最顶层的包。
        - 子包中的类可以访问父包中的内容，无需导入。但外层是不能直接访问内层的，需要导入。同样的，父包也可以调自包中的对象属性。
...
- scala中的公有属性，底层实际为`private`，并通过get方法`obj.field()`和set方法`obj.field_=(value)`对其进行操作。所以scala不推荐设置为`private`。如果需要和其他框架互操作，必须提供Java Bean规范的getter和setter的话可以加`@scala.beans.BeanProperty`注解。
...
- 包括主构造器和辅助构造器。
```scala
class ClassName [descriptor] [([权限修饰descriptor][val/var] arg1: Arg1Type, [descriptor][val/var] arg2: ...)] { // main constructor, only one, like record in java
    // assist constructor
    def this(argsList1) {
...
```
在类的形参列表中可以出现三种情况：①没有，就是一个空参的主构造器；②不加val和var，形参就是局部变量；③加上val和var，就声明成了一个主构造器。辅助构造器中必须要在第一行有this（主构造器形参列表）。
...
// 1.情况一。需要在内部声明成员变量，然后再通过辅助构造器赋值（情况23略了）
class Person {
    var name: String = _
    var age: Int = _
...
用法大致和java一致，需要注意的是构造器的问题，子类使用构造器必须调用父类构造器，顺序是辅助构造器 -找-主构造器 -找- 父类构造器
...
- **子类继承父类时，形参列表就不需要var和val了**

```scala
class Son(name:String, weight:Int) extends InitTest(name, weight){
  println("1.这是儿子的主构造器")
}
```
...
- java中属性静态绑定。举个例子，java父类中声明了多态方法来打印所有成员变量。然后利用多态new一个子类赋给父类对象，然后调用该多态方法。那么打印的该是父类的属性信息还是子类的？答父类，因为java中属性静态绑定，只有方法是适用多态，方法是动态绑定。
- 但scala中**属性和方法都是动态绑定**。就上面那个例子而言，输出的就是子类的属性信息了。
...
- 子类调用父类中方法使用`super`关键字。
  - 子类**重写父类抽象属性**，父类抽象属性可以用`var`修饰，`val var`都可以。因为父类没有实现嘛，需要到子类中来实现。
...
- 其中可以定义抽象和非抽象的属性和方法。
- 特征和父类或者多个特征中重名的属性或方法需要在子类中覆写以解决冲突。
    - 如果是方法冲突，最后因为动态绑定，所有使用的地方都是子类的字段或方法。
    - 属性冲突，按照顺序最后以一个定义了该属性的
    - 属性的话需要类型一致，不然提示不兼容。方法的话参数列表不一致会视为重载而不是冲突。
...
println(arr)
// 6. remove element
arr.remove(0, 1) // startIndex, count删几个数
println(arr)
...
// traverse
arr.foreach(println(_.mkString(","))) 
...
- 操作基于源集合做更改。
- 为了与不可变集合区分，**命名相同但是包位置是不同的**，`import scala.collection.mutable`并用`mutable.Set`。
- 不可变集合有的都有。
...
- 需要注意默认情况处理，`case _`也需要返回值
- 通过指定匹配变量的类型（用特定类型变量接住），可以匹配类型而不匹配值，也可以混用。
- 需要注意类型匹配时由于**泛型擦除**，可能并不能严格匹配泛型的类型参数，编译器也会报警告。但`Array`是基本数据类型，对应于java的原生数组类型，能够匹配泛型类型参数。
...
协变和逆变：
- 比如`Son`和`Father`是父子关系，`Son`是子类，拿集合类型当作整体来看。
